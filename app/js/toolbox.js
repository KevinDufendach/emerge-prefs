...
function KrdToolbox($q) {
  "use strict";

  this.PromiseManager = function() {
    var promiseList = [];
    var count = 0;
    var success = true;

    this.addPromise = function(newPromise) {
      promiseList.add(newPromise);
    ...
    this.runAsync = function() {
      return $q(function(resolve, reject) {
        ...
        // run each promise asynchronously, decrementing count as each completes
        // When count equals 0, complete the runAsync.
        for (var i = 0; i < promiseList.length; i++) {
          promiseList[i].then(
            // successful completion decrements counter
            function() {
              count--;

              if (count == 0) complete();
            },
            // unsuccessful completion decrements counter, sets success flag to false
            function() {
              count--;
              success = false;
